...
<changes>
Corrigir erro na função de overview financeiro
Replacing
<old_str>
// Calculate overdue receivables
      const overdueReceivables = receivables.filter(ar => {
        if (ar.status === 'paid' || ar.status === 'cancelled') return false;
        return new Date(ar.dueDate) < new Date() && compareMoney(ar.amount, ar.receivedAmount) > 0;
      }).reduce((total, order) => {
        const remaining = subtractMoney(order.amount, order.receivedAmount);
        return addMoney(total, remaining);
      }, "0.00");
</old_str>
with
<new_str>
// Calculate overdue receivables
      const overdueReceivables = receivables.filter(ar => {
        ...
        const remaining = subtractMoney(ar.amount, ar.receivedAmount);
        return addMoney(total, remaining);
      }, "0.00");
</new_str>
</changes>

The code block below has the corrected code.
<replit_final_file>
import type { Express } from "express";
import { createServer, type Server } from "http";
import multer from 'multer';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/logistics/products/import", upload.single('file'), async (req, res) => {
    try {
      ...
      if (productsData.length > 10000) {
        return res.status(400).json({
          error: "Muitosprodutos no arquivo. O limite é de 10.000 produtos por importação."
        });
      }
